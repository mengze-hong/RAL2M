...
parser.add_argument("--input_file", type=str, default="../5_Analysis/debate_part_positive.jsonl")
parser.add_argument("--output_file", type=str, default="../5_Analysis/debate_part_negative.jsonl")
args = parser.parse_args()

# ------------------- PROMPT DESIGN -------------------
# Using a clear structure that forces the model to start with '['
DEBATE_PROMPT_TEMPLATE = """You are a professional service agent. For the following query and a candidate response QA, generate a one-sentence explanation justifying WHY the candidate QA is a bad response to the user query and should NOT be returned as a response. Constraint: Return ONLY a single sentence. No markdown.
...
# ------------------- LLM 判断函数（批量） -------------------
def debate_retrieval_batch(batch_rows):
    prompts = []
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            max_new_tokens=100,
            temperature=0.1,  # fixed: avoids sampling disabled error
            top_p=1.0,
            do_sample=False,
...
print("Starting qwen-7B relevance debate on retrieved top-1...")
with open(args.input_file, "r", encoding="utf-8") as f:
    for line in tqdm(f, desc="Judging retrieval"):
        ...
        if len(batch) >= args.batch_size:
            debate = debate_retrieval_batch(batch)
            for row, jud in zip(batch, debate):
                row = row.copy()
                row["qwen_debate_negative"] = jud
...
print("New column: qwen_debate_retrieval (True = retrieved answer is correct, False = should reject)")