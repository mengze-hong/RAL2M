# llm_rejudge_retrieval_with_llama8.py
# Use llama-3-8B-Instruct to judge: "Is the retrieved top-1 passage correct for this query?"
# Output: adds column → llama_judge_retrieval: True / False
...
parser.add_argument("--input_file", type=str, default="judgement_results/final_results.jsonl")
parser.add_argument("--output_file", type=str, default="judgement_results/final_results_llama.jsonl")
args = parser.parse_args()

# ------------------- 模型加载 -------------------
LOCAL_MODEL_DIR = os.path.expanduser("~/autodl-tmp/models")
llama_PATH = os.path.join(LOCAL_MODEL_DIR, "llama-3.1-8b-instruct")
print("加载 llama2.5-7B-Instruct...")
...
print("Starting llama-7B relevance judgment on retrieved top-1...")
    ...
    for line in tqdm(f, desc="Judging retrieval"):
        ...
        if len(batch) >= args.batch_size:
            ...
            for row, jud in zip(batch, judgements):
                row = row.copy()
                row["llama_judge_retrieval_without_role"] = jud
...
print("New column: llama_judge_retrieval (True = retrieved answer is correct, False = should reject)")