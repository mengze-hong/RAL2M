# llm_rejudge_retrieval_with_mistral.py
# Use mistral-3-8B-Instruct to judge: "Is the retrieved top-1 passage correct for this query?"
# Output: adds column → mistral_judge_retrieval: True / False
...
parser.add_argument("--input_file", type=str, default="judgement_results/new_results_updated_new.jsonl")
parser.add_argument("--output_file", type=str, default="judgement_results/new_results_updated_new_new.jsonl")
args = parser.parse_args()

# ------------------- 模型加载 -------------------
LOCAL_MODEL_DIR = os.path.expanduser("~/autodl-tmp/models")
mistral_PATH = os.path.join(LOCAL_MODEL_DIR, "mistralai-Mistral-7B-Instruct-v0.3")
print("加载 mistral-7B-Instruct...")
tokenizer = AutoTokenizer.from_pretrained(mistral_PATH, local_files_only=True, trust_remote_code=True)
...
print("Starting mistral-7B relevance judgment on retrieved top-1...")
    ...
    for line in tqdm(f, desc="Judging retrieval"):
        ...
        if len(batch) >= args.batch_size:
            ...
            for row, jud in zip(batch, judgements):
                row = row.copy()
                row["mistral_judge_retrieval_without_role"] = jud
...
print("New column: mistral_judge_retrieval (True = retrieved answer is correct, False = should reject)")