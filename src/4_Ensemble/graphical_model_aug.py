...
SAVE_DIR = "graphical_model_aug"

if not os.path.exists(SAVE_DIR):
    os.makedirs(SAVE_DIR)

JUDGE_COLS = ["qwen_aug", "llama_aug", 
              "mistral_aug", "gemma_aug", 
              "chatglm_aug"]
K = len(JUDGE_COLS)
...
# BEST: 2025-12-23 00:46:32, 0.7138, 0.1684, 512, 0.3, 0.001, 0.5, 10, 256, 0.1, 2.0, 0.1 

# param_grid = {
#     'HIDDEN_DIM': 512,
#     'DROPOUT': 0.3,
#     'LR': 1e-3,  # Ensure LR is in your grid if you use it in the loop
#     'GRAD_CLIP':  0.5,
#     'VI_TRAIN_ITERS': 10,
#     'MC_SAMPLES_TRAIN': 256,
#     'LABEL_SMOOTHING': 0.1,
#     'FOCAL_GAMMA': 2.0,
#     'KL_WEIGHT': 0.1
# }
...
for epoch in range(EPOCHS):
    ...
    if acc > best_acc or (np.isclose(acc, best_acc, atol=1e-4) and hallu < best_hallu):
        ...
        best_hallu = hallu
        best_epoch = epoch + 1
        torch.save(mu_net.state_dict(), os.path.join(SAVE_DIR, "best_mu_net.pth"))
        torch.save(sigma_net.state_dict(), os.path.join(SAVE_DIR, "best_sigma_net.pth"))
        torch.save(potentials.state_dict(), os.path.join(SAVE_DIR, "best_potentials.pth"))

print(f"\nTraining Complete. Best Epoch: {best_epoch} | Best Acc: {best_acc:.4f} | Best Hallu: {best_hallu:.4f}")

# ==================== FINAL EVALUATION ====================
# Load best weights
mu_net.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_mu_net.pth")))
sigma_net.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_sigma_net.pth")))
potentials.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_potentials.pth")))
