# retrieval_top100_compact.py
# 高效、干净、工业级 —— 每条 query 一行，top-100 id 存 list
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "train_with_top100_ids.csv")  # 改名避免覆盖
...
# ------------------- 主流程：为每条 query 取 top-100 id -------------------
print("\n开始检索，为每条 query 生成 top-100 D1 id 列表...")
...
for idx, row in tqdm(train_df.iterrows(), total=len(train_df), desc="Retrieval"):
    ...
    # 关键：k=100
    scores, indices = index.search(q_emb.astype('float32'), k=100)
    
    top100_ids = [d1_ids[i] for i in indices[0]]
    top100_scores = scores[0].tolist()
    
    # 构造新行
    new_row = row.to_dict()
    new_row['top100_d1_ids'] = top100_ids
    new_row['top100_scores'] = top100_scores
    new_row['top1_is_correct'] = row['true_q_i'] in d1_df[d1_df['id'].isin(top100_ids)]['q_i'].values
    new_row['top10_is_correct'] = any(
        row['true_q_i'] == d1_df[d1_df['id'] == str(d1_ids[i])]['q_i'].iloc[0] 
        for i in indices[0][:10]
    ) if len(indices[0]) >= 10 else False
...
print(f"   Top-1  召回率: {result_df['top1_is_correct'].mean():.2%}")
print(f"   Top-10 召回率: {result_df['top10_is_correct'].mean():.2%}")

# ------------------- 统计报告 -------------------
stats = {
    'total_queries': len(result_df),
    'top1_recall': result_df['top1_is_correct'].mean(),
    'top10_recall': result_df['top10_is_correct'].mean(),
    'avg_top1_score': result_df['top100_scores'].apply(lambda x: x[0]).mean(),
}
print("\n检索性能总结（可直接进论文）：")
for k, v in stats.items():
    print(f"  {k:15}: {v:.4f}" if isinstance(v, float) else f"  {k:15}: {v}")

print(f"\n下一歩：用 top100_d1_ids 去生成批量 LLM judgments（我随时给你最强版本）")